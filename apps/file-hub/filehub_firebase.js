// File Hub Logic - Firebase Storage Version
let currentUser = null;
let allFiles = [];
let currentFilter = 'all';

// 1. Initialize Firebase
const firebaseApp = firebase.initializeApp(window.ENV.FILE_HUB_FIREBASE_CONFIG);
const storage = firebase.storage();

document.addEventListener('DOMContentLoaded', () => {
    // Get user from Portal (via URL param)
...
function initEventListeners() {
    ...
    if (openBtn) {
        openBtn.disabled = false; // Always enabled for Firebase
...
async function handleFileUpload(e) {
    ...
    try {
        ...
        const fileRef = storage.ref().child(`file-hub/${currentUser}/${Date.now()}_${file.name}`);
        const snapshot = await fileRef.put(file);
        const fileUrl = await snapshot.ref.getDownloadURL();

        // 2. Save Metadata to Supabase via GAS
        const gasPayload = {
            action: 'uploadFileMetadata',
            username: currentUser,
            fileName: file.name,
            driveFileId: snapshot.ref.fullPath, // Use Firebase path as ID
            fileUrl: fileUrl,
            isPublic: isPublic,
            category: category
        };
